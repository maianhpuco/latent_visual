...
class QwenWithLVR(Qwen2_5_VLForConditionalGeneration):
    ...
    def _resolve_hidden_size(self) -> int:
        """
        Get the LLM hidden_size robustly across transformers versions.

        Qwen2_5_VLConfig layout varies by transformers version:
          - Flat layout (some versions): config.hidden_size exists directly.
          - Nested layout (newer):        config.text_config.hidden_size
          - Broken property (some builds): text_config getter itself raises.
        Fallback: read from lm_head weight shape, which is always valid after
        super().__init__() has completed.
        """
        try:
        ...
        try:
            ...
            pass
        # lm_head = nn.Linear(hidden_size, vocab_size) — always present
    ...
    def _init_lvr_head(self, lvr_head_type):
        ...
        if lvr_head_type == 'simple':
            self.lvr_head = LVRHead(hidden_size=hidden_size)
        elif lvr_head_type == 'glu':
            # Retrieve FFN params with safe fallbacks
            try:
                cfg = self.config if hasattr(self.config, 'intermediate_size') else self.config.text_config
                intermediate_size = cfg.intermediate_size
                hidden_act = cfg.hidden_act
            except AttributeError:
                # Read from model weights: gate_proj.out_features == intermediate_size
                intermediate_size = self.model.layers[0].mlp.gate_proj.out_features
                hidden_act = 'silu'
    ...
    def _init_lvr_latent_end_emb(self):
        ...
        hidden_size = self._resolve_hidden_size()
        with torch.no_grad():
            v = torch.randn(hidden_size, dtype=self.dtype, device=self.device)
            v = v / (v.norm() + 1e-6)
            v = v * (target_norm_scale_latentend * math.sqrt(hidden_size))
    ...
    def generate(
        ...
        **kwargs,
        ...
        # generation_mode already resolved above for cache prep

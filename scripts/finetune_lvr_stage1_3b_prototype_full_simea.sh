...
# Simea server — Prototype-based parallel LVR, FULL fine-tuning (GPUs 4,5,6,7)
# Full FT: entire LLM updated (no LoRA). Prototype modules trained from scratch.
...
export WANDB_PROJECT="LVR-Qwen25-VL-3B-SFT-STAGE-1-PROTOTYPE-FULL"
...
MAX_STEPS=5000
BATCH_PER_DEVICE=2
GRAD_ACCUM_STEPS=2   # 4 GPUs × 2 accum × 2 batch = effective batch 16

# Learning rates
# Full FT uses lower LR than LoRA (entire LLM is plastic, larger risk of catastrophic forgetting)
LR=1e-5             # LLM (full fine-tuning)
PROTOTYPE_LR=5e-4   # PrototypeBank P and g_φ (randomly initialised, needs higher LR)
...
LOSS_DIV_LAMBDA=0.05
LOSS_FOCUS_LAMBDA=0.01

# Warmup: prototype modules only for first 500 steps
...
RUN_NAME="Stage1_3B_Prototype_FullFT_K${NUM_PROTOTYPES}_div${LOSS_DIV_LAMBDA}_focus${LOSS_FOCUS_LAMBDA}-Steps${MAX_STEPS}"
OUTPUT_DIR="checkpoints_rerun/stage1_3b_prototype_full/"

START_TIME=$(date +%s)
echo "[start] $(date) — run: $RUN_NAME"
echo "[mode]  FULL fine-tuning (no LoRA) — entire LLM trainable at lr=${LR}"
echo "[gpu]   GPUs 4,5,6,7 on simea"
echo "[proto] K=${NUM_PROTOTYPES} prototypes, parallel inference, ∂z_k/∂z_j=0"
echo "[eta]   MAX_STEPS=$MAX_STEPS | ~4s/step on 4 GPUs (flash_attn2+ZeRO3-no-offload) → ETA ~$((MAX_STEPS * 4 / 3600))h"

$CONDA_ENV/bin/python \
    -m deepspeed.launcher.runner --include $GPU_INCLUDE --master_addr 127.0.0.1 --master_port 29504 src/train/train_lvr_prototype.py \
    --run_name "$RUN_NAME" \
    --coconut True \
    --prototype_mode True \

...
# Simea server — DIMV-style latent reasoning fine-tuning
# Sequence layout: [x_txt | V | Z | Y]
# Only NTP loss; information bottleneck enforced by the 4D attention mask.
# LoRA on LLM backbone + full fine-tuning of LatentReasoningModule.
...
export WANDB_PROJECT="LVR-Qwen25-VL-3B-DIMV-STAGE1"
...
# LLM learning rate (LoRA params)
LR=1e-4
# LatentReasoningModule learning rate (full FT)
LATENT_REASONING_LR=1e-4
...
RUN_NAME="DIMV_3B_2GPU_Slots${NUM_SLOTS}_L${NUM_REFINE_STEPS}_LoRAr${LORA_RANK}-Steps${MAX_STEPS}"
OUTPUT_DIR="checkpoints_dimv/stage1_3b_dimv/"

START_TIME=$(date +%s)
echo "[start]  $(date) — run: $RUN_NAME"
echo "[mode]   DIMV — NTP-only loss, attention-mask bottleneck"
...
echo "[batch]  ${BATCH_PER_DEVICE} per device × ${GRAD_ACCUM_STEPS} accum × 4 GPUs = effective batch $((BATCH_PER_DEVICE * GRAD_ACCUM_STEPS * 4)) | packed_tokens=${MAX_PACKED_TOKENS}"
echo "[resume] ${RESUME_FROM:-none (fresh start)}"

$CONDA_ENV/bin/python \
    -m deepspeed.launcher.runner --include "$GPU_INCLUDE" --master_addr 127.0.0.1 --master_port 29504 src/train/train_lvr_lora.py \
    --run_name "$RUN_NAME" \
    --coconut False \
    --dimv_mode True \
